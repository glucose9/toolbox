  ...
  privacy: {
    ko: {
      ...
      sections: [
        ...
        { h: "분석 도구", p: ["사이트 개선을 위해 Vercel Analytics(쿠키를 사용하지 않는 익명 집계)와 Google Analytics를 사용할 수 있습니다. 방문 페이지, 대략적 위치(국가 수준), 기기·브라우저 종류 등 익명화된 통계가 수집됩니다. 개별 사용자를 식별하지 않습니다.", "Google의 데이터 처리에 대해서는 Google 개인정보처리방침을 참고하세요."] },
    ...
    en: {
      ...
      sections: [
        ...
        { h: "Analytics", p: ["We may use Vercel Analytics (cookieless, anonymous, aggregate) and Google Analytics to improve the site. It collects anonymized statistics such as pages visited, approximate location (country level), and device/browser type. It does not identify individual users.", "See Google's Privacy Policy for how Google processes this data."] },
    ...
    ja: {
      ...
      sections: [
        ...
        { h: "分析ツール", p: ["サイト改善のためVercel Analytics(Cookieを使わない匿名集計)とGoogle Analyticsを使用する場合があります。訪問ページ、おおよその位置(国レベル)、端末・ブラウザの種類など匿名化された統計を収集します。個別の利用者を識別しません。", "Googleのデータ処理についてはGoogleプライバシーポリシーをご参照ください。"] },
    ...
    zh: {
      ...
      sections: [
        ...
        { h: "分析工具", p: ["我们可能使用 Vercel Analytics(无Cookie的匿名汇总)和 Google Analytics 来改进网站。它收集匿名统计信息,如访问页面、大致位置(国家级别)、设备/浏览器类型等,不识别个人用户。", "有关Google如何处理这些数据,请参阅Google隐私政策。"] },
