...
export const tools: ToolConfig[] = [
  ...
  { slug: "pdf-to-docx", component: "PdfToDocxTool", category: "document", icon: "📄", navTitle: "PDF → DOCX", title: "PDF → 워드(DOCX) 변환 - 무료 (브라우저 처리)", h1: "PDF → 워드(DOCX) 변환기", description: "PDF의 텍스트를 추출해 워드(.docx) 문서로 변환합니다. 본문 텍스트와 단락 구분이 보존되며, 워드에서 바로 열어 추가 편집이 가능합니다. 단순 텍스트 위주 PDF(보고서 본문, 계약서 본문 등)에 적합. 파일은 브라우저 안에서만 처리, 업로드 없음.", metaDescription: "PDF → DOCX 무료 변환. 워드에서 편집 가능, 브라우저 처리.", howTo: ["PDF 파일을 드래그하거나 선택합니다.", "텍스트가 추출되어 워드 형식으로 변환됩니다.", "[DOCX 다운로드]를 누릅니다."], faq: [{ q: "원본 PDF 레이아웃이 그대로 보존되나요?", a: "아닙니다. 본문 텍스트와 단락 구분 중심으로 변환되며, 복잡한 다단·표·이미지 위치·정밀한 폰트 서식은 단순화되거나 빠질 수 있습니다. iLovePDF·SmallPDF가 정밀한 이유는 서버에서 OCR과 ML 레이아웃 분석을 하기 때문이고, 본 도구는 브라우저 내 텍스트 추출 기반이라 차이가 있습니다." }, { q: "어떤 PDF에 잘 맞나요?", a: "한 단(1-column) 본문 위주의 PDF(보고서 본문, 책 본문, 계약서 본문). 학술 논문·잡지·복잡한 양식은 흐트러질 수 있습니다." }, { q: "스캔본 PDF도 되나요?", a: "안 됩니다. 스캔된 이미지 PDF는 OCR이 먼저 필요합니다. [이미지 OCR] 도구로 글자를 추출한 뒤 [TXT → PDF] / [Markdown → PDF]를 사용하세요." }, { q: "표·이미지는?", a: "표 구조와 이미지는 현재 변환되지 않습니다(텍스트만). 표가 핵심이면 [PDF → XLSX]를, 이미지가 핵심이면 [PDF → 이미지 변환]을 검토하세요." }, { q: "파일이 외부로 가나요?", a: "아니요. pdfjs로 브라우저 내에서 텍스트를 추출하고 docx 라이브러리로 워드 파일을 만듭니다. 어디로도 전송되지 않습니다." }, { q: "한글·일문·중문은?", a: "유니코드 텍스트는 그대로 보존됩니다." }], addedAt: "2026-06-02" },
  { slug: "pdf-to-xlsx", component: "PdfToXlsxTool", category: "document", icon: "📊", navTitle: "PDF → XLSX", title: "PDF → 엑셀(XLSX) 변환 - 표를 시트로 (무료, 단순 PDF용)", h1: "PDF → 엑셀(XLSX) 변환기", description: "PDF에서 텍스트를 추출해 엑셀(.xlsx) 시트로 변환합니다. 페이지별로 시트를 분리하거나, 라인 단위로 셀에 채워 후처리하기 좋은 형태로 저장. 단순 표 위주 PDF(가격표·점수표·재무 데이터)에 적합. 브라우저 안에서 처리.", metaDescription: "PDF → 엑셀 무료. 페이지별 시트, 단순 표 PDF용, 브라우저 처리.", howTo: ["PDF 파일을 업로드합니다.", "각 페이지의 텍스트가 시트(또는 셀 단위)로 추출됩니다.", "[XLSX 다운로드]를 누릅니다."], faq: [{ q: "복잡한 표도 잘 인식되나요?", a: "아닙니다. 정밀한 표 인식은 서버 ML이 필요한 영역이고, 본 도구는 텍스트 추출 기반이라 깔끔한 표 PDF에만 잘 맞습니다. 격자선 없이 공백으로 정렬된 표나, 셀이 병합된 복잡한 양식은 어긋날 수 있습니다." }, { q: "어떤 PDF에 잘 맞나요?", a: "격자 구조가 명확한 단순 표(가격표, 시간표, 단순 재무표). 다단·이미지·복잡한 양식은 부정확합니다." }, { q: "스캔본 PDF도 되나요?", a: "안 됩니다. OCR이 먼저 필요합니다. [이미지 OCR]을 사용한 뒤 텍스트를 정리하세요." }, { q: "셀이 잘못 나뉘면?", a: "텍스트 위치 기반으로 추출되므로 PDF의 띄어쓰기에 따라 셀 경계가 달라질 수 있습니다. 다운로드 후 엑셀에서 수동 정리를 권장합니다." }, { q: "파일이 외부로 가나요?", a: "아니요. pdfjs와 SheetJS가 브라우저 내에서 처리합니다." }, { q: "정밀한 PDF 표 추출이 필요해요.", a: "표 추출은 서버 OCR/ML이 정확합니다(Adobe Acrobat, iLovePDF 등 유료 서비스). 본 도구는 빠른 1차 추출용입니다." }], addedAt: "2026-06-02" },
  { slug: "pdf-to-pptx", component: "PdfToPptxTool", category: "document", icon: "📊", navTitle: "PDF → PPTX", title: "PDF → 파워포인트(PPTX) 변환 - 페이지별 슬라이드 이미지 (무료)", h1: "PDF → PPTX 변환기 (이미지 기반)", description: "PDF의 각 페이지를 고화질 이미지로 렌더링한 뒤 파워포인트(.pptx) 슬라이드 1장씩에 배치합니다. 원본 모양 그대로 보존되며, 파워포인트에서 열어 텍스트 박스를 위에 얹어 주석을 달거나 다른 슬라이드 사이에 끼워 사용하기 좋습니다. 이미지 기반이라 텍스트 편집은 안 됩니다.", metaDescription: "PDF → PPTX 무료. 페이지별 이미지 슬라이드, 원본 모양 그대로 보존.", howTo: ["PDF 파일을 업로드합니다.", "각 페이지가 고해상도 이미지로 슬라이드에 배치됩니다.", "[PPTX 다운로드]를 누릅니다.", "파워포인트에서 열어 텍스트 박스나 도형을 추가할 수 있습니다."], faq: [{ q: "텍스트를 편집할 수 있나요?", a: "이미지 기반 변환이라 직접 편집은 안 됩니다. 텍스트로 편집하고 싶으면 [PDF → DOCX]나 [PDF → Markdown]을 사용하세요. 본 도구는 '원본 모양 보존'이 강점입니다." }, { q: "왜 이미지 기반인가요?", a: "PDF를 파워포인트의 도형·텍스트 박스로 완벽히 재구성하는 건 서버 ML이 필요합니다. 본 도구는 원본 모양 100% 보존을 우선해 페이지를 이미지로 슬라이드에 박습니다. 발표 자료의 일부 페이지를 슬라이드에 끼워 넣을 때 유용." }, { q: "슬라이드 방향은?", a: "원본 PDF 페이지 비율에 맞춰 자동 결정됩니다. 일반 A4 PDF는 세로, 와이드 PDF는 가로." }, { q: "고해상도로 나오나요?", a: "기본 2배 스케일로 렌더링되어 큰 화면 발표에서도 깨끗합니다." }, { q: "파일이 외부로 가나요?", a: "아니요. pdfjs로 렌더링하고 pptxgenjs가 .pptx를 만들며, 모두 브라우저 안에서 처리됩니다." }, { q: "페이지 수 제한?", a: "수십 페이지까지 무난합니다. 100페이지 이상이면 생성에 시간이 걸립니다." }], addedAt: "2026-06-02" },
  { slug: "pdf-to-html", component: "PdfToHtmlTool", category: "document", icon: "🌐", navTitle: "PDF → HTML", title: "PDF → HTML 변환 - 웹페이지 형식으로 추출 (무료)", h1: "PDF → HTML 변환기", description: "PDF의 텍스트와 단락을 HTML 문서로 추출합니다. 웹사이트에 내용을 옮기거나, 검색·복사가 쉬운 형태로 보관할 때 사용. 페이지 구분, 기본 단락 구조 보존. 평이한 본문 위주 PDF에 적합. 브라우저 안에서 처리.", metaDescription: "PDF → HTML 무료. 웹페이지 형식으로 변환, 브라우저 처리.", howTo: ["PDF 파일을 업로드합니다.", "텍스트가 HTML 단락으로 변환됩니다.", "[HTML 다운로드]를 누르거나 복사해서 사용합니다."], faq: [{ q: "원본 레이아웃이 보존되나요?", a: "단락 구분과 기본 텍스트는 보존되지만, 정밀한 위치·폰트·다단 레이아웃은 단순화됩니다. 본문 위주의 평이한 PDF에 가장 잘 맞습니다." }, { q: "이미지도 추출되나요?", a: "현재는 텍스트 위주이며, 이미지는 빠질 수 있습니다. 이미지가 핵심이면 [PDF → 이미지 변환] 도구를 사용하세요." }, { q: "어떤 용도에 적합?", a: "PDF 내용을 블로그·위키·웹사이트에 옮길 때, 검색 가능한 형태로 보관할 때, 마크다운/HTML 후처리를 위한 1차 추출용." }, { q: "복잡한 표는?", a: "텍스트로 추출되지만 표 구조는 잘 보존되지 않습니다. 표 위주 PDF는 [PDF → XLSX]를 검토하세요." }, { q: "파일이 외부로 가나요?", a: "아니요. pdfjs가 브라우저에서 텍스트를 추출하고 HTML을 만듭니다." }, { q: "스캔본 PDF도 되나요?", a: "안 됩니다. 텍스트 레이어가 없는 스캔 PDF는 [이미지 OCR]이 먼저 필요합니다." }], addedAt: "2026-06-02" },
  { slug: "pdf-to-md", component: "PdfToMdTool", category: "document", icon: "📝", navTitle: "PDF → Markdown", title: "PDF → 마크다운 변환 - 깃허브·블로그용 .md 추출 (무료)", h1: "PDF → 마크다운(.md) 변환기", description: "PDF의 텍스트를 마크다운(.md) 형식으로 추출합니다. 폰트 크기로 헤딩(#, ##) 추정, 단락 구분, 글머리 기호 인식. 깃허브 README·블로그·위키 옮기기, 마크다운 노트 후처리 1차 작업에 유용. 브라우저 안에서 처리.", metaDescription: "PDF → 마크다운 무료. 헤딩·단락 자동 인식, GitHub·블로그용.", howTo: ["PDF 파일을 업로드합니다.", "텍스트가 마크다운으로 변환됩니다 (헤딩·단락 자동 인식).", "복사하거나 [.md 다운로드]를 누릅니다."], faq: [{ q: "헤딩(#)은 어떻게 인식?", a: "PDF 본문의 폰트 크기를 분석해 가장 큰 글자를 #, 그 다음을 ## 식으로 추정합니다. 표지·로고·일부 PDF는 잘못 인식될 수 있어 다운로드 후 수동 조정을 권장합니다." }, { q: "원본 레이아웃이 보존?", a: "마크다운 자체가 단순 서식 언어라 PDF의 정밀 레이아웃을 표현하지 못합니다. 단락·헤딩·목록 정도까지만 보존됩니다." }, { q: "어떤 PDF에 잘 맞나요?", a: "한 단(1-column) 본문 위주의 PDF, 블로그·문서 형태의 자료. 학술 논문·잡지·복잡한 양식엔 부적합." }, { q: "스캔본 PDF도 되나요?", a: "안 됩니다. OCR이 먼저 필요합니다." }, { q: "코드 블록·표는?", a: "고정폭으로 보이는 블록은 코드 블록으로 추정, 표는 가능한 한 파이프(|) 표로 변환을 시도하지만 완벽하지 않습니다." }, { q: "파일이 외부로 가나요?", a: "아니요. 모두 브라우저 안에서 처리됩니다." }], addedAt: "2026-06-02" },
