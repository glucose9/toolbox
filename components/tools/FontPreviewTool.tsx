...
export default function FontPreviewTool() {
  const t = useTranslations("toolUI.font-preview");
  ...
  return (
    <div className="card space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3 text-sm items-end">
        <label>{t("size")} ({size}px)<input type="range" min="10" max="80" value={size} onChange={(e) => setSize(+e.target.value)} className="w-full" /></label>
        <label>{t("weight")} ({weight})<input type="range" min="100" max="900" step="100" value={weight} onChange={(e) => setWeight(+e.target.value)} className="w-full" /></label>
      </div>
      ...
      <details className="rounded border border-gray-200 dark:border-gray-700">
        ...
        <div className="p-3 pt-1">
          <label className="block text-sm">{t("font")}<input type="text" value={font} onChange={(e) => setFont(e.target.value)} className="w-full mt-1 px-2 py-1 border border-gray-200 dark:border-gray-700 rounded bg-white dark:bg-gray-900" /></label>
        </div>
      </details>
