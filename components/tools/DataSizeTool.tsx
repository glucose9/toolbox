...
const ALL_UNITS = [...UNITS_SI, ...UNITS_IEC.filter((x) => x.n !== "B")];
...
export default function DataSizeTool() {
  ...
  const u = ALL_UNITS.find((x) => x.n === unit) || ALL_UNITS[0];
    ...
    <div className="card space-y-3">
      <div className="grid grid-cols-2 gap-3">
        ...
        <select value={unit} onChange={(e) => setUnit(e.target.value)} className="px-3 py-2 border border-gray-200 dark:border-gray-700 rounded bg-white dark:bg-gray-900">
          {ALL_UNITS.map((u) => <option key={u.n}>{u.n}</option>)}
        </select>
      </div>
      <div className="grid grid-cols-2 gap-3">
        {[{ label: t("modeSi"), list: UNITS_SI }, { label: t("modeIec"), list: UNITS_IEC }].map((col) => (
          <div key={col.label} className="border border-gray-200 dark:border-gray-700 rounded divide-y divide-gray-200 dark:divide-gray-700">
            <div className="px-3 py-2 text-sm font-medium">{col.label}</div>
            {col.list.map((x) => (
