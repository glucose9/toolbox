...
export default function DDayTool() {
  ...
  const result = (() => {
    if (mode === "dday") {
      const tg = dateOnly(target);
      if (!tg) return null;
      const diff = Math.round((tg.getTime() - today.getTime()) / DAY_MS);
      const label = diff === 0 ? "D-DAY" : diff > 0 ? `D-${diff}` : `D+${Math.abs(diff)}`;
      return (
        <div>
          <div className="text-3xl font-bold">{label}</div>
          <div className="text-sm text-muted mt-1">{fmt(tg)}</div>
          <div className="text-sm mt-1">
            {diff > 0 ? t("daysLeft", { n: diff }) : diff < 0 ? t("daysPassed", { n: Math.abs(diff) }) : t("isToday")}
          </div>
        </div>
      );
    }
    if (mode === "diff") {
      ...
      return (
        <div>
          <div className="text-3xl font-bold">{t("daysUnit", { n: absDays.toLocaleString() })}</div>
          <div className="text-sm text-muted mt-1">
            {t("weeksDays", { w: weeks, d: restDays, y: (absDays / 365.25).toFixed(2) })}
          </div>
          <div className="text-sm mt-1">
            {fmt(a)} → {fmt(b)} {days < 0 ? t("reverse") : ""}
          </div>
        </div>
      );
    }
  ...
  return (
    <div className="card space-y-4">
      <div className="flex flex-wrap gap-2">
        ...
        <button onClick={() => setMode("diff")} className={`btn ${mode === "diff" ? "btn-primary" : "btn-secondary"}`}>{t("modeDiff")}</button>
        <button onClick={() => setMode("after")} className={`btn ${mode === "after" ? "btn-primary" : "btn-secondary"}`}>{t("modeAfter")}</button>
      </div>

      {mode === "dday" && (
        <div>
          <label className="label">{t("targetDate")}</label>
      ...
      {mode === "after" && (
        <div className="grid grid-cols-2 gap-2">
          <div>
            <label className="label">{t("baseDate")}</label>
          ...
          <div>
            <label className="label">{t("daysInput")}</label>
