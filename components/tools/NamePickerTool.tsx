...
export default function NamePickerTool() {
  const t = useTranslations("toolUI.name-picker");
  ...
  return (
    <div className="card space-y-3">
      <div>
        ...
        />
        <div className="text-xs text-muted mt-1">{t("poolCount", { count: pool.length })}</div>
      </div>
      ...
      <div className="flex flex-wrap gap-3 items-end text-sm">
        ...
        <button onClick={draw} disabled={pool.length === 0 || spinning} className="btn btn-primary disabled:opacity-50">{t("draw")}</button>
      </div>
      ...
      {results.length > 0 && (
        <div className={`bg-brand-50 dark:bg-brand-900/20 border border-brand-200 dark:border-brand-800 rounded p-4 ${spinning ? "animate-pulse" : ""}`}>
          <div className="text-xs text-muted mb-2">{t("winners", { count: results.length })}</div>
